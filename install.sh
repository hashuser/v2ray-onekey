...
web_camouflage(){
    ...
    pathing=$[$[$RANDOM % 5] + 1] 
    wget https://github.com/hashuser/v2ray-onekey/blob/master/template/$pathing.zip?raw=true -O /home/webtemp/$pathing.zip
...
v2ray_install(){
    ...
    if [[ -f install-release.sh ]];then
        bash install-release.sh --version v4.36.2
        judge "安装 V2ray"
    else
